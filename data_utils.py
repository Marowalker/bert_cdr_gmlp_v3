...
def dot_simililarity_dim1(x, y):
    ...
    v = tf.matmul(x, tf.transpose(y, [0, 2, 1]))
    v = tf.reduce_mean(v, axis=[1, 2], keepdims=True)
    # v = tf.tensordot(x, tf.transpose(y, [0, 2, 1]), axes=0)
    return v


def dot_simililarity_dim2(x, y):
    # v = tf.tensordot(tf.expand_dims(x, 1), tf.expand_dims(tf.transpose(y), 0), axes=2)
    v = tf.tensordot(x, tf.transpose(y, [1, 2, 0]), axes=2)
    # x shape: (N, 1, C)
    # y shape: (1, C, 2N)
    # v shape: (N, 2N)
    return v
